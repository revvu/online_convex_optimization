    ...
    for run in run_iter:
        ...
        for ti, T in enumerate(t_iter):
            ...
            for rep in range(replicates):
                ...
                # Unpack comparator tuple: (vector, loss)
                u_star_vec, _ = comparator_ftl_hindsight(z, y, R)

                reps_vals["FTRL"].append(
                    simulate_alg(z, y, u_star_vec, alg_flag=0, R=R, eta0=math.sqrt(2))
                )
                reps_vals["FTL"].append(
                    simulate_alg(z, y, u_star_vec, alg_flag=1, R=R, eta0=math.sqrt(2))
                )
                reps_vals["SMART"].append(
                    simulate_SMART(z, y, u_star_vec, R=R)
                )
                reps_vals["EMP"].append(
                    simulate_empirical_g_SMART(z, y, u_star_vec, float(g_emp[int(T)]), R=R)
                )
